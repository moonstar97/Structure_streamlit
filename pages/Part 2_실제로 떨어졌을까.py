...
df_agg = pd.read_csv("data/grade_percent.csv", encoding="cp949")
...
if selected_school == "중학교":
    fig1_1 = px.line(df_agg, x="연도", y="3수준비율(중)"
    ...
    fig1_2 = px.line(df_agg, x="연도", y="3수준비율(고)"
                , color="과목", markers=True)
...
st.markdown("## 2.2. 학업성취도 열등학생 비율")

if selected_school == "중학교":
    fig2_1 = px.line(df_agg, x="연도", y="1수준비율(중)"
                , color="과목", markers=True)
    ...
    fig2_2 = px.line(df_agg, x="연도", y="1수준비율(고)"
                , color="과목", markers=True)
